...
torchrun \
    ...
    agent.vlm_path='/path/to/internvl3_8b_finetune_full_pretrain' \
    agent.cam_type='single' \
    agent.grpo=False \
    agent.cache_hidden_state=True \
    agent.vlm_type="internvl" \
    agent.dit_type="small" \
    cache_path="/path/to/exp/recogdrive_agent_cache_dir_train_test" \
    use_cache_without_dataset=True \
    agent.sampling_method="ddim" \
    experiment_name=recogdrive_agent_eval > eval.txt 2>&1
